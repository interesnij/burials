...
pub async fn main_search(req: HttpRequest) -> actix_web::Result<HttpResponse> {
    ...
    if params_some.is_ok() {
        ...
        if params.birth_date.is_some() && params.birth_date.as_deref().unwrap().format("%Yd-%m-%d").to_string() == "2023-11-25" {
            birth_date = None;
        }
        else {
            birth_date = params.birth_date;
        }
        if params.death_date.is_some() && params.death_date.as_deref().unwrap().format("%Yd-%m-%d").to_string() == "2023-11-25" {
            death_date = None;
        }
        else {
